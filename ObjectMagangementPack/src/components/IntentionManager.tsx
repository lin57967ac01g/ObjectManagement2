...
import { CreateEditIntentionModal, TestRunModal } from './IntentionModals';
import svgPaths from "../imports/svg-nze44xlz6d";

// --- Mock Data ---
const initialIntentions: Intention[] = [
    { id: '1', name: 'Intention001', description: 'Accounting with 2026 financial plan', flowId: 'f1', dataSubject: 'Finance', owner: 'K. Y. Lin' },
    { id: '2', name: 'Intention002', description: 'Business KPI summary', flowId: 'f2', dataSubject: 'Business', owner: 'T. L. Liao' },
    { id: '3', name: 'Intention003', description: 'Technology development', flowId: 'f3', dataSubject: 'Technology', owner: 'T. L. Liao' },
];
...
interface IntentionManagerProps {
    intentions: Intention[];
    flows: Flow[];
    onSave: (intention: Intention) => void;
}

export function IntentionManager({ intentions, flows, onSave }: IntentionManagerProps) {
  const [modalMode, setModalMode] = useState<'none' | 'create' | 'edit' | 'run'>('none');
  const [selectedIntention, setSelectedIntention] = useState<Intention | undefined>(undefined);
  ...
  const handleEdit = (item: Intention) => {
      setSelectedIntention(item);
      setModalMode('edit');
  };

  const handleRun = (item: Intention) => {
      setSelectedIntention(item); // Pre-select in run modal if needed, or just use it as context
      setModalMode('run');
  };

  const handleSaveInternal = (data: Intention) => {
      onSave(data);
      setModalMode('none');
  };
    ...
    <div className="absolute bg-white h-[1020px] left-[320px] top-[60px] right-0 bottom-0 w-[calc(100%-320px)] overflow-auto border-l border-[#cfcfcf]">
      <div className="box-border content-stretch flex flex-col gap-[10px] h-full items-start px-[30px] py-[20px] relative w-full">
        ...
        <div className="flex justify-between w-full items-center">
            <p className="font-bold text-[32px] text-black">Intention Management</p>
            <button 
            ...
            >
                <PlusIcon />
                <span className="text-[20px] text-neutral-100">Create Intention</span>
            </button>
        </div>
        ...
        <div className="bg-white border border-[#cfcfcf] rounded-[4px] w-full flex flex-col">
            {/* Header */}
            <div className="grid grid-cols-[200px_1fr_200px_180px_180px_174px] bg-[#e8e8e8] border-b border-[#cfcfcf] h-[55px]">
                <div className="border-r border-[#cfcfcf] flex items-center px-3 text-[18px] text-[#484848] font-semibold">Name</div>
                <div className="border-r border-[#cfcfcf] flex items-center px-3 text-[18px] text-[#484848] font-semibold">Intention</div>
                <div className="border-r border-[#cfcfcf] flex items-center px-3 text-[18px] text-[#484848] font-semibold">Linked Flow</div>
                <div className="border-r border-[#cfcfcf] flex items-center px-3 text-[18px] text-[#484848] font-semibold">Data Subject</div>
            ...
            {/* Rows */}
            {intentions.map((item) => (
                <div key={item.id} className="grid grid-cols-[200px_1fr_200px_180px_180px_174px] border-b border-[#cfcfcf] min-h-[55px] hover:bg-gray-50">
                    ...
                    <div className="border-r border-[#cfcfcf] flex items-center px-3 text-[18px] text-[#484848]">
                        {flows.find(f => f.id === item.flowId)?.name || item.flowId}
                    </div>
                    <div className="border-r border-[#cfcfcf] flex items-center px-3 text-[18px] text-[#484848]">{item.dataSubject}</div>
